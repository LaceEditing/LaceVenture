...
# Terminal colors - Lavender theme with Unicode escape sequences
class Colors:
    DM_NAME = '\u001b[38;5;141m'  # Medium lavender, for DM name
    DM_TEXT = '\u001b[38;5;183m'  # Light lavender, for DM text
    PLAYER = '\u001b[38;5;219m'  # Light purple/pink, for player
    SYSTEM = '\u001b[38;5;99m'  # Deep lavender, for system messages
    HIGHLIGHT = '\u001b[38;5;177m'  # Bright lavender, for highlights
...
def colored_print(text, color, slow=False, add_padding=False):
    """Print text with color, with optional padding for better spacing"""
    colored_text = f"{color}{text}{Colors.RESET}"

    if add_padding:
        print()  # Add a blank line before
        print(colored_text)
        print()  # Add a blank line after
    else:
        print(colored_text)


def print_divider(width=60, style="•"):
    """Print a decorative divider"""
    divider = style * width
    print(f"\n{Colors.HIGHLIGHT}{divider}{Colors.RESET}\n")
...
def print_header(text, width=60):
    ...
    print(f"\n{Colors.HIGHLIGHT}{header}{Colors.RESET}\n")
...
def create_new_story():
    ...
    print_header("CREATE NEW STORY", 60)

    player_input = {}

    # Get available models and let player choose
    available_models = get_available_ollama_models()
    print_header("SELECT AI MODEL", 40)
    ...
    while True:
        model_choice = input(f"\n{Colors.PLAYER}Choose a model (number or name): {Colors.RESET}")
    ...
    print_divider(width=40, style="-")
    player_input["story_title"] = input(f"{Colors.PLAYER}Story title: {Colors.RESET}")
    player_input["world_name"] = input(f"{Colors.PLAYER}World name: {Colors.RESET}")
    player_input["genre"] = input(f"{Colors.PLAYER}Genre (fantasy, sci-fi, western, etc.): {Colors.RESET}")
    player_input["setting"] = input(f"{Colors.PLAYER}Setting description: {Colors.RESET}")
    player_input["tone"] = input(f"{Colors.PLAYER}Tone (dark, lighthearted, mysterious, etc.): {Colors.RESET}")

    # Get content rating
    print_header("CONTENT RATING", 40)
    ...
    while True:
        rating = input(f"\n{Colors.PLAYER}Choose a rating (E/T/M): {Colors.RESET}").upper()
        if rating in ["E", "T", "M"]:
    ...
    while True:
        pace = input(
            f"\n{Colors.PLAYER}Choose plot pacing (Fast-paced/Balanced/Slice-of-life): {Colors.RESET}").capitalize()
    ...
    print_header("CHARACTER CREATION", 40)
    player_input["character_name"] = input(f"{Colors.PLAYER}Your character's name: {Colors.RESET}")
    player_input["character_race"] = input(f"{Colors.PLAYER}Your character's race: {Colors.RESET}")
    player_input["character_class"] = input(f"{Colors.PLAYER}Your character's class/profession: {Colors.RESET}")

    traits = input(f"{Colors.PLAYER}Character traits (comma separated): {Colors.RESET}")
    if traits:
        player_input["character_traits"] = [t.strip() for t in traits.split(",")]

    abilities = input(f"{Colors.PLAYER}Character abilities (comma separated): {Colors.RESET}")
    if abilities:
        player_input["abilities"] = [a.strip() for a in abilities.split(",")]

    # Get starting location
    print_header("STARTING LOCATION", 40)
    player_input["starting_location_name"] = input(f"{Colors.PLAYER}Starting location name: {Colors.RESET}")
    player_input["starting_location_description"] = input(
        f"{Colors.PLAYER}Starting location description: {Colors.RESET}")

    # Get initial quest
    print_header("INITIAL QUEST", 40)
    player_input["quest_name"] = input(f"{Colors.PLAYER}Initial quest name: {Colors.RESET}")
    player_input["quest_description"] = input(f"{Colors.PLAYER}Initial quest description: {Colors.RESET}")

    # Get world facts
    print_header("WORLD FACTS", 40)
    facts = input(f"{Colors.PLAYER}Enter some facts about your world (comma separated): {Colors.RESET}")
    if facts:
        player_input["world_facts"] = [f.strip() for f in facts.split(",")]

    # Get NPCs (optional)
    print_header("NPCS (Optional)", 40)
    colored_print("Would you like to add NPCs? (Type 'done' when finished)", Colors.SYSTEM)

    npcs = []
    while True:
        npc_name = input(f"\n{Colors.PLAYER}NPC name (or 'done' to finish): {Colors.RESET}")
        if npc_name.lower() == 'done':
            break

        npc = {"name": npc_name}
        npc["race"] = input(f"{Colors.PLAYER}NPC race: {Colors.RESET}")
        npc["description"] = input(f"{Colors.PLAYER}NPC description: {Colors.RESET}")
        npc["disposition"] = input(f"{Colors.PLAYER}NPC disposition (friendly, hostile, neutral): {Colors.RESET}")
        npc["motivation"] = input(f"{Colors.PLAYER}NPC motivation: {Colors.RESET}")
        npc["dialogue_style"] = input(f"{Colors.PLAYER}NPC dialogue style: {Colors.RESET}")
...
def manage_stories():
    ...
    if not stories:
        colored_print("No stories found to manage.", Colors.SYSTEM, add_padding=True)
        return

    print_header("MANAGE STORIES", 50)
    ...
    while True:
        choice = input(f"\n{Colors.PLAYER}Enter story number to delete (or 'cancel'): {Colors.RESET}")
        ...
        if choice.isdigit():
            ...
            if 1 <= choice_num <= len(stories):
                ...
                # Confirm deletion
                confirm = input(
                    f"{Colors.PLAYER}Are you sure you want to delete '{story_title}'? (yes/no): {Colors.RESET}")
                if confirm.lower() == 'yes':
                    if delete_story(file_name):
                        colored_print(f"Story '{story_title}' deleted successfully.", Colors.SYSTEM, add_padding=True)
                        return
                else:
...
def handle_game():
    ...
    print_header("DUNGEON MASTER AI", 60)
    ...
    choice = input(f"\n{Colors.PLAYER}Choose an option (1-4): {Colors.RESET}")

    if choice == "4":
        colored_print("Goodbye!", Colors.SYSTEM, add_padding=True)
        return
    ...
    if choice == "1":
        ...
        colored_print("Creating your world...", Colors.SYSTEM, add_padding=True)
        ...
        stories = list_stories()
        if not stories:
            colored_print("No existing stories found.", Colors.SYSTEM, add_padding=True)
            return

        print_header("AVAILABLE STORIES", 50)

        for i, (file_name, story_title) in enumerate(stories, 1):
            colored_print(f"{i}. {story_title} [{file_name}]", Colors.SYSTEM)
        story_choice = input(f"\n{Colors.PLAYER}Choose a story number or type its name: {Colors.RESET}")
        ...
        if story_choice.isdigit() and 1 <= int(story_choice) <= len(stories):
            ...
            if not matched:
                colored_print("Story not found.", Colors.SYSTEM, add_padding=True)
                return

        # Load the selected story
        game_state = load_game_state(file_name)
        if not game_state:
            colored_print("Error loading story.", Colors.SYSTEM, add_padding=True)
            return

        story_name = game_state['game_info']['title']
        colored_print(f"Loaded story: {story_name}", Colors.SYSTEM, add_padding=True)
        ...
        colored_print("Invalid choice.", Colors.SYSTEM, add_padding=True)
        return

    # Main game loop
    colored_print("Type 'exit' or 'quit' to leave the game.", Colors.SYSTEM)
    ...
    while True:
        ...
        if player_input.lower() in ['exit', 'quit']:
            save_game_state(game_state, story_name)
            colored_print("Game saved. Goodbye!", Colors.SYSTEM, add_padding=True)
            break

        if player_input.lower() == 'save':
            save_game_state(game_state, story_name)
            colored_print("Game saved!", Colors.SYSTEM, add_padding=True)
            continue

        if player_input.lower() == 'summary':
            # Generate narrative summary
            print_header("THE STORY SO FAR", 50)
            ...
            for paragraph in paragraphs:
                ...
                if "**" in paragraph:
                    ...
                    for i, part in enumerate(parts):
                        ...
                        if i % 2 == 1:
                            colored_print(part, Colors.HIGHLIGHT)  # Highlight important parts
                        else:
        ...
        if player_input.lower() == 'memory':
            print_header("NARRATIVE MEMORY", 50)
            ...
            print()  # Add spacing
            colored_print("Character development:", Colors.SYSTEM)
            for item in memory['character_development']:
                colored_print(f"• {item}", Colors.SYSTEM)

            print()
            colored_print("Relationships:", Colors.SYSTEM)
            for item in memory['relationships']:
                colored_print(f"• {item}", Colors.SYSTEM)

            print()
            colored_print("Plot developments:", Colors.SYSTEM)
            for item in memory['plot_developments']:
                colored_print(f"• {item}", Colors.SYSTEM)

            print()
            colored_print("Important player decisions:", Colors.SYSTEM)
            for item in memory['player_decisions']:
                colored_print(f"• {item}", Colors.SYSTEM)

            print()
            colored_print("Environment details:", Colors.SYSTEM)
            for item in memory.get('environment_details', []):
                colored_print(f"• {item}", Colors.SYSTEM)

            print()
            colored_print("Conversation details:", Colors.SYSTEM)
        ...
        if game_state['important_updates']:
            print_header("IMPORTANT DEVELOPMENTS", 40)
            for update in game_state['important_updates']:
                colored_print(f"✧ {update}", Colors.HIGHLIGHT)
